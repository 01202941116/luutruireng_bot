...
def sanitize_filename(name: str) -> str:
    """Làm sạch tên file để lưu đĩa / DB an toàn."""
...
async def save_file_to_db(
    ...
    """Tải file từ Telegram, lưu BLOB vào DB, trả về file_db_id."""
    user = update.effective_user
    message = update.message
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        ...
        if param.startswith("folder"):
            ...
            if not folder:
                await message.reply_text(
                    "Không tìm thấy thư mục (có thể đã xoá)."
                )
                return
...
def main():
    ...
    # Khởi tạo DB
    db.init_db()
    print(Fore.GREEN + "DB SQLite đã được khởi tạo." + Fore.RESET)
    ...
    print(Fore.BLUE + "Bot is running..." + Fore.RESET)
