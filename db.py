...
def get_or_create_folder(owner_id, name):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
...
def get_folders_by_owner(owner_id):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
...
def search_folders(owner_id, keyword):
    kw = f"%{keyword}%"
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
        "SELECT * FROM folders WHERE owner_telegram_id = ? AND name LIKE ? "
        "ORDER BY id DESC",
        (owner_id, kw),
    )
    rows = cur.fetchall()
...
def insert_file(
    ...
    cur.execute(
        """
        INSERT INTO files (
            owner_telegram_id, folder_id, file_type, file_unique_id, file_id,
            filename, file_blob, file_size, mime_type
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
...
def get_last_file_by_owner(owner_id):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
