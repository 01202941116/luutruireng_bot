...
def set_user_approved(telegram_id: int, is_approved: bool):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
        "UPDATE users SET is_approved = ? WHERE telegram_id = ?",
        (1 if is_approved else 0, telegram_id),
    )
    conn.commit()
    conn.close()
...
def get_last_file_by_owner(owner_id: int):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
